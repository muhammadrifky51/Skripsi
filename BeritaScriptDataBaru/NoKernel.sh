...
do
	OutFoldTop="HasilTopicDetectionBerita-${DimRed^^}"
	...
	do
		...
		do
			...
			python3 Processinge.py $CleanOutFile $i1 "0" $DimRed $OutFoldTop $OutFileTop $timing
			cd ../Coherence
			topic_file="../BeritaScriptDataBaru/$OutFoldTop/$OutFileTop"
			oc_file="../BeritaScriptDataBaru/results/berita-${DimRed^^}/$OutFileTop"
		...
		#theresult="results/$DimRed/berita1-$hour-$i1.txt"
		#python3 HitungRataan.py "results/berita-${DimRed^^}" $hour $i1 > $theresult
	done
done 
